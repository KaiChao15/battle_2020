...
def safezone(src, grid, failsafe):
    ...
    if 0 < up['y'] <= height:
        if grid[up['y']-1][up['x']-1] == '.' or grid[up['y']-1][up['x']-1] == 'O':
            safe.append(up)

    if 0 < down['y'] <= height:
        if grid[down['y'] - 1][down['x'] - 1] == '.' or grid[down['y'] - 1][down['x'] - 1] == 'O':
            safe.append(down)

    if 0 < right['x'] <= width:
        if grid[right['y'] - 1][right['x'] - 1] == '.' or grid[right['y'] - 1][right['x'] - 1] == 'O':
            safe.append(right)

    if 0 < left['x'] <= width:
        if grid[left['y'] - 1][left['x'] - 1] == '.' or grid[left['y'] - 1][left['x'] - 1] == 'O':
            safe.append(left)
        ...
        if maxPlayer:
            for i in range(len(moves)):
                ...
                if (body_length > 1 and
                    ...
                    print("grid checking y: ", new_state['me']['body'][body_length]['y'])
                    print("grid checking x: ", new_state['me']['body'][body_length]['x']-1)
                    new_grid[new_state['me']['body'][body_length]['y']][new_state['me']['body'][body_length]['x']-1] = '.'
            ...
            for i in range(len(moves)):
                ...
                if (body_length > 1 and
                    ...
                    print("grid checking y: ", new_state['target']['body'][body_length]['y'])
                    print("grid checking x: ", new_state['target']['body'][body_length]['x']-1)
                    new_grid[new_state['target']['body'][body_length]['y']][new_state['target']['body'][body_length]['x']-1] = '.'
